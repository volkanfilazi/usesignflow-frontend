...
export class GenericFormDetailComponent implements OnInit, OnDestroy {
  buildingForm = true;
  ...
  private buildFormControls(
    ...
    fields.forEach((field) => {
      ...
      if (field.type === 'agreement') {
        const controlName = `${field.fieldId}_accepted`;

        const existingValue = answers?.find((x) => x.fieldId === controlName)?.value ?? false;
      ...
      const existingValue = answers?.find((x) => x.fieldId === field.fieldId)?.value ?? null;
  ...
  create(): void {
    ...
    const answers = fields.map((field) => ({
      ...
      value: this.normalizeAnswerValue(this.myGroup.get(field.fieldId)?.value ?? null),
    }));
      ...
      this.formApiService
        ...
        .subscribe({
          next: () => {
            ...
            //this.downloadPdf(this.form?.id!, this.externalUserToken!);
          },
          error: () => {
            this.loading$.next(false);
            this.toolsService.showSnackbar(
